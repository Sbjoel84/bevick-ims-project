...
@media print { @page { size:A4; margin:14mm 12mm; } body { font-size:12px; } .page { padding:0; } }
`;

// ── Print Receipt ─────────────────────────────────────────────────────────────

export function printReceipt(sale, state) {
  const { currency = 'NGN', bizName, bizRC, bizPhone, bizEmail, bizAddress } = state || {};
...
<div class="wrap">
  <div class="head">
    ...
    ${bizPhone   ? `<div class="biz-sub">Tel: ${esc(bizPhone)}</div>` : ''}
  </div>
...
export function printReport({ title, subtitle, columns, rows, summaryRows, dateRange, state }) {
  const { bizName, bizRC, bizPhone } = state || {};
