...
import { fmtDateTime, formatCurrency } from '../context/AppContext';
import { printReport } from '../utils/print';
import { exportLedgerToExcel, exportLedgerToCSV } from '../utils/excel';
import { fetchTransactions, fetchReportSummary } from '../lib/salesTransactionService';
import { getDateRange } from '../utils/dateRanges';

const BRANCHES = [{ id: 'DUB', label: 'Dubai Market' }, { id: 'KUB', label: 'Kubwa Office' }];
const TRANSACTION_TYPES = ['Sale Created', 'Sale Updated', 'Sale Deleted', 'Payment Recorded', 'Booking Delivered', 'Sale Restored'];
...
const TYPE_STYLES = {
  'Sale Created': 'bg-green-950 text-green-400',
  'Sale Updated': 'bg-blue-950 text-blue-400',
  'Sale Deleted': 'bg-red-950 text-red-400',
  'Payment Recorded': 'bg-cyan-950 text-cyan-400',
  'Booking Delivered': 'bg-indigo-950 text-indigo-400',
  'Sale Restored': 'bg-emerald-950 text-emerald-400',
};
...
export default function SalesTransactionsLedger({ state, canExport }) {
  ...
  const columns = [
    ...
    { key: 'amount_before', label: 'Amount Before', align: 'tr', format: v => formatCurrency(v || 0, currency) },
    { key: 'amount_changed', label: 'Amount Changed', align: 'tr', format: v => formatCurrency(v || 0, currency) },
    { key: 'amount_after', label: 'Amount After', align: 'tr', format: v => formatCurrency(v || 0, currency) },
  ...
  const { filters, updateFilters, resetFilters, page, setPage, pageSize, rows, total, loading, summary } = useSalesTransactions();
  ...
  async function buildReportPayload() {
    ...
    const summaryRows = [
      ...
      { label: 'Total Sales Value', value: formatCurrency(reportSummary?.total_sales_value || 0, currency) },
      { label: 'Total Payments Recorded', value: formatCurrency(reportSummary?.total_payments || 0, currency) },
      { label: 'Most Active Officer', value: reportSummary?.most_active_officer || '—' },
      { label: 'Most Active Customer', value: reportSummary?.most_active_customer || '—' },
    ];
  ...
  async function handlePrintReport() {
    ...
    try {
      ...
      printReport({
        title: 'Sales Transaction Ledger', subtitle: state.bname || 'All Branches',
        columns, rows: reportRows, summaryRows, dateRange: dateRangeLabel, state,
  ...
  async function handleExport(kind) {
    ...
    try {
      ...
      if (kind === 'excel') {
        exportLedgerToExcel({
          rows: reportRows, columns, bizName: state.bizName, bizRC: state.bizRC,
          title: 'Sales Transaction Ledger', dateRangeLabel, generatedBy: state.user?.name,
          summaryRows, fileNamePrefix: 'Sales-Transactions',
        });
      } else {
        exportLedgerToCSV({ rows: reportRows, columns, fileNamePrefix: 'Sales-Transactions' });
      }
    } finally {
      setReportBusy(false);
    }
  ...
  function printSingleRow(row) {
    printReport({
      ...
      columns, rows: [row], summaryRows: null, dateRange: null, state,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <SummaryCard label="Month Sales Value" value={formatCurrency(summary?.month_sales_value || 0, currency)} accent="text-green-400" />
        <SummaryCard label="Month Payments" value={formatCurrency(summary?.month_payments || 0, currency)} accent="text-cyan-400" />
        <SummaryCard label="Month Deletions" value={summary?.month_deletions ?? '—'} accent="text-red-400" />
      </div>
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={row.id} className="border-b border-gray-800 last:border-0 hover:bg-gray-800/40 transition-colors">
                  ...
                  <td className="px-3 py-2.5 font-mono text-gray-400 text-xs whitespace-nowrap">{row.sale_id || '—'}</td>
                  <td className="px-3 py-2.5 text-white font-medium whitespace-nowrap">{row.customer_name || '—'}</td>
                  ...
                  <td className="px-3 py-2.5 text-right font-mono text-gray-400">{formatCurrency(row.amount_before || 0, currency)}</td>
                  <td className={`px-3 py-2.5 text-right font-mono font-semibold ${row.amount_changed > 0 ? 'text-green-400' : row.amount_changed < 0 ? 'text-red-400' : 'text-gray-500'}`}>
                    {formatCurrency(row.amount_changed || 0, currency)}
                  </td>
                  <td className="px-3 py-2.5 text-right font-mono text-white">{formatCurrency(row.amount_after || 0, currency)}</td>
                  <td className="px-3 py-2.5 text-gray-400 whitespace-nowrap">{row.payment_method || '—'}</td>
                  ...
                  <td className="px-3 py-2.5 whitespace-nowrap">
                    <span className="text-xs font-medium px-2 py-0.5 rounded-lg bg-gray-800 text-gray-300">{row.status}</span>
                  </td>
