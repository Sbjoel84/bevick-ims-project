...
export default function Sales() {
  ...
  // Profit profile visibility in the new-sale form (controls cost-price inputs)
  ...
  // Per-row profit reveal — tracks which sale IDs have profit details expanded
  const [profitVisible, setProfitVisible] = useState(new Set());
  function toggleRowProfit(id) {
    setProfitVisible(prev => {
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredSales.length === 0 ? (
                ...
                const showProfit = profitVisible.has(s.id);
                return (
                  <>
                    <tr key={s.id} className="border-b border-gray-800 last:border-0 hover:bg-gray-800/40 transition-colors">
                      ...
                      <td className="px-4 py-3.5">
                        <div className="flex items-center gap-2 justify-end">
                          <button
                            onClick={() => toggleRowProfit(s.id)}
                            className={`transition-colors ${showProfit ? 'text-green-400' : 'text-gray-500 hover:text-green-400'}`}
                    ...
                    {showProfit && (
                      <tr key={`${s.id}-profit`} className="border-b border-gray-800 bg-gray-800/30">
                        <td colSpan={7} className="px-6 py-2.5">
                          <div className="flex flex-wrap gap-6 text-xs">
                            <span className="flex items-center gap-1.5">
                              <span className="text-gray-500">Actual Cost</span>
                              <span className="text-amber-400 font-mono font-semibold">{s.totalCost != null ? formatCurrency(s.totalCost, currency) : '—'}</span>
                            </span>
                            {s.totalDiscount > 0 && (
                              <span className="flex items-center gap-1.5">
                                <span className="text-gray-500">Discount</span>
                                <span className="text-orange-400 font-mono font-semibold">−{formatCurrency(s.totalDiscount, currency)}</span>
                              </span>
                            )}
                            {s.totalCommission > 0 && (
                              <span className="flex items-center gap-1.5">
                                <span className="text-gray-500">Commission</span>
                                <span className="text-purple-400 font-mono font-semibold">−{formatCurrency(s.totalCommission, currency)}</span>
                              </span>
                            )}
                            <span className="flex items-center gap-1.5">
                              <span className="text-gray-500">Net Profit</span>
                              <span className={`font-mono font-bold ${s.profit != null ? (s.profit >= 0 ? 'text-green-400' : 'text-red-400') : 'text-gray-600'}`}>
          ...
          {filteredSales.length === 0 ? (
            ...
            return (
              <div key={s.id} className="p-4 space-y-3">
                ...
                {showProfit && (
                  <div className="flex flex-wrap gap-3 text-xs bg-gray-800/50 rounded-xl px-3 py-2.5">
                ...
                <div className="flex items-center gap-2 justify-end pt-2 border-t border-gray-800">
                  <button
                    onClick={() => toggleRowProfit(s.id)}
                    className={`p-2 transition-colors ${showProfit ? 'text-green-400' : 'text-gray-500 hover:text-green-400'}`}
                    title={showProfit ? 'Hide profit' : 'Show profit'}
                  >
                    <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z"/></svg>
                  </button>
